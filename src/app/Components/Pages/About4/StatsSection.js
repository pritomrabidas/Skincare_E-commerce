...
const StatsSection = () => {
  ...
  return (
    <section className=" absolute -bottom-3 left-8 container mx-auto px-4 py-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
        {stats.map((stat, index) => (
          ...
          >
            <h3 className="text-3xl font-bold font-Monrope text-primary">{stat.value}</h3>
            <p className="text-gray-600 text-base font-Popins font-normal mt-2">{stat.title}</p>
          </div>
        ))}
      </div>
    </section>
